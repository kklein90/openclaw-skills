...
def _arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Optional JSON file path to load/save tokens. Do not place it in a git repo or synced folder.",
    )
    parser.add_argument(
        "--auto-refresh",
        action="store_true",
...
def _upload_image_bytes(upload_url: str, image_path: str) -> None:
    ...
    if parsed.scheme != "https":
        raise ValueError("Refusing non-HTTPS upload URL from LinkedIn initializeUpload response.")
    ...
    if (not tokens["access_token"] and args.auto_refresh) or (
        ...
        try:
            ...
            if args.persist_client_secret:
                token_store["client_secret"] = tokens["client_secret"]
            else:
                token_store.pop("client_secret", None)
